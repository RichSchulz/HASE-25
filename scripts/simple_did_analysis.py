...
def load_and_prepare_data(pre_period_weeks=4):
    """
...
def prepare_did_variables(df, treatment_period='two_weeks', pre_period_weeks=4):
    ...
    if treatment_period == 'two_weeks':
        ...
        daily_user_stats['post_treatment'] = ((daily_user_stats['date'] >= april_1st) & 
                                            (daily_user_stats['date'] < end_post)).astype(int)
    elif treatment_period == 'four_weekdays':
        ...
        daily_user_stats['post_treatment'] = ((daily_user_stats['date'] >= start_post) & 
                                            (daily_user_stats['date'] < end_post) &
...
def main():
    ...
    for period in treatment_periods:
        ...
        df = prepare_did_variables(df_raw.copy(), treatment_period=period)
