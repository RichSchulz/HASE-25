...
def run_regression(df, outcome_var, outcome_name):
    ...
    # Note: In TWFE models, treatment and post_treatment are absorbed into fixed effects
    # Only the interaction (treatment_post) and group-specific trends are identifiable
    ...
    try:
        ...
        print_detailed_results(results, outcome_name, absorb_cols, df)
...
def create_did_visualization(df, outcome_var, outcome_name, period, treatment_start_date, output_dir, regression_result=None):
    ...
    # (treatment_time_trend) and absorbed fixed effects, which means that
    # the counterfactual post value would not be perfectly parallel, since it
    # would have to account for the time trend and fixed effects.
    # We use this simplified approach to illustrate the DiD effect.
    ...
    italy_values = [italy_pre, treatment_post_y]
    ...
    ax.text(1, treatment_post_y, f'  {treatment_post_y:.3f}', va='bottom', ha='left', fontsize=10, fontweight='bold', color='#1F77B4')
    ...
    arrow_y_bottom = min(counterfactual_post, treatment_post_y)
    arrow_y_top = max(counterfactual_post, treatment_post_y)
    arrow_mid = (arrow_y_bottom + arrow_y_top) / 2
    
    # Draw double-headed arrow (grey/black instead of green)
    arrow = FancyArrowPatch(
        (1.15, counterfactual_post), (1.15, treatment_post_y),
    ...
    all_values = [italy_pre, treatment_post_y, control_pre, control_post, counterfactual_post]
    ...
    filename = f"did_before_after_{outcome_var}_{period_clean}.png"
...
def main():
    ...
    # Export Table
    output_path = f"{Path(__file__).resolve().parent.parent.parent}/final report/parts/did_regression_table.tex"
