...
(Fetches commits from users which are located in Italy, Austria or France)
"""
...
def db_worker(request_queue: Queue, response_queue: Queue):
    job_conn = sqlite3.connect(f"{Path(__file__).resolve().parent.parent}/large_data/commits_all.sqlite3", check_same_thread=False)
    job_conn.row_factory = sqlite3.Row

    files_conn = sqlite3.connect(f"{Path(__file__).resolve().parent.parent}/large_data/data_files.sqlite3", check_same_thread=False)
    commits_conn = sqlite3.connect(f"{Path(__file__).resolve().parent.parent}/large_data/data_commits.sqlite3", check_same_thread=False)
