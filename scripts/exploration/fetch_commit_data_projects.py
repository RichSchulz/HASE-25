...
def db_worker(request_queue: Queue, response_queue: Queue):
    job_conn = sqlite3.connect(f"{Path(__file__).resolve().parent}/large_data/commits_projects.sqlite3", check_same_thread=False)
    job_conn.row_factory = sqlite3.Row

    files_conn = sqlite3.connect(f"{Path(__file__).resolve().parent}/large_data/data_files_projects.sqlite3", check_same_thread=False)
    commits_conn = sqlite3.connect(f"{Path(__file__).resolve().parent}/large_data/data_commits_projects.sqlite3", check_same_thread=False)
