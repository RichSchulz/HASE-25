...
def commits_to_sqlite():
    ...
    for country in countries:
        country_df = csv_to_df(
            f"{Path(__file__).resolve().parent}/data/only_filtered_manually_projects_{country}.csv",
            {
                'country': country,
                'started_at': pd.NaT,
                'completed_at': pd.NaT,
    ...
    if merged is not None:
        df_to_sqlite(merged, f"{Path(__file__).resolve().parent}/large_data/commits_projects.sqlite3", 'commits')
